...
if ($bln == 'all') {
    $sql = $koneksi->query("SELECT * FROM barang_keluar WHERE YEAR(tanggal) = '$thn' ORDER BY tanggal DESC");
    $judul = "Laporan Barang Keluar Tahun $thn";
    $periode = "Tahun $thn";
} else {
    $sql = $koneksi->query("SELECT * FROM barang_keluar WHERE MONTH(tanggal) = '$bln' AND YEAR(tanggal) = '$thn' ORDER BY tanggal DESC");
    ...
    $judul = "Laporan Barang Keluar Bulan $nama_bulan Tahun $thn";
...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->SetFillColor(231, 76, 60); // Merah untuk barang keluar
        ...
        $this->SetXY(10, 28);
        $this->Cell(277, 8, 'LAPORAN BARANG KELUAR', 0, 1, 'C');
    ...
    function InfoBox($total_data, $total_jumlah) {
        ...
        // Card 2 - Total Item
        $this->SetFillColor(230, 126, 34); // Orange untuk barang keluar
        $this->Rect(105, $y_start + 5, 70, 15, 'F');
        $this->SetTextColor(255, 255, 255);
        $this->SetFont('Arial', 'B', 12);
        $this->SetXY(110, $y_start + 8);
        $this->Cell(60, 4, 'Total Barang Keluar', 0, 1, 'C');
        $this->SetFont('Arial', 'B', 14);
...
// Background header tabel dengan warna merah untuk barang keluar
$pdf->SetFillColor(231, 76, 60); // Merah header
$pdf->SetTextColor(255, 255, 255);
$pdf->SetDrawColor(192, 57, 43);
$pdf->SetLineWidth(0.5);

// Header columns dengan lebar yang disesuaikan
$pdf->Cell(20, 12, 'No', 1, 0, 'C', true);
$pdf->Cell(40, 12, 'ID Transaksi', 1, 0, 'C', true);
$pdf->Cell(35, 12, 'Tanggal Keluar', 1, 0, 'C', true);
...
$pdf->Ln(2);
$pdf->SetFont('Arial', 'B', 11);
$pdf->SetFillColor(230, 126, 34); // Orange untuk summary barang keluar
$pdf->SetTextColor(255, 255, 255);
$pdf->SetDrawColor(211, 84, 0);

$pdf->Cell(235, 12, 'TOTAL KESELURUHAN BARANG KELUAR', 1, 0, 'C', true);
...
$pdf->Cell(277, 5, 'Catatan: Laporan ini menampilkan seluruh data barang keluar pada periode ' . $periode, 0, 1, 'L');
...
$filename = 'Laporan_Barang_Keluar_' . ($bln == 'all' ? 'Tahun_'.$thn : 'Bulan_'.$bln.'_'.$thn) . '_' . $timestamp . '.pdf';
