...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['login'])) {
    ...
    if ($data) {
        ...
        // Cek apakah hash di database sudah format bcrypt/password_hash (pakai $2y$)
        $valid = false;
        ...
        if ($valid && $data['level'] === $level) {
            // Buat session sesuai level
            if ($level == 'superadmin') {
            ...
            $error = "Login gagal. Password atau Level salah.";
        }
    } else {
